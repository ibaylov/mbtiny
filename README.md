...
It is strange how many applications begin with usage of a hash table or other kind of map to implement registers logic whereas the addresses of variables seldom change in run time even in controllers that support this.  Using of conditionals instead of map erduces the memory footprint and makes the implementation viable even on smallest RAM devices. Usage of macros hides the implementation details from the programmer and simplifies greatly the readability of the code making it future proof.
